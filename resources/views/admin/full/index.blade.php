...
@section('title', trans('admin.full.actions.index'))

@section('body')

<full-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/fulls') }}'" inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{ trans('admin.full.actions.index') }}
                    <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2" href="{{ url('admin/fulls/export') }}"
                        role="button"><i class="fa fa-file-excel-o"></i>&nbsp;
                        {{ trans('admin.full.actions.export') }}</a>
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/fulls/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.full.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.full.columns.id') }}</th>
                                    <th is='sortable' :column="'pacient_id'">
                                        {{ trans('admin.full.columns.pacient_id') }}</th>
                                    <th is='sortable' :column="'data_examen'">
                                        {{ trans('admin.full.columns.data_examen') }}</th>
                                    <th is='sortable' :column="'data_examen_mod'">
                                        {{ trans('admin.full.columns.data_examen_mod') }}</th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                        <span
                                            ...
                                            @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/fulls')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/fulls/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in collection" :key="item.id"
                                    ...
                                    <td>@{{ item.id }}</td>
                                    <td>@{{ item.pacient_id }}</td>
                                    <td>@{{ item.data_examen | datetime }}</td>
                                    <td>@{{ item.data_examen_mod | datetime }}</td>
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/fulls/create') }}"
